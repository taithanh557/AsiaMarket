...
const Health: React.FC = () => {
  const [healthProducts, setHealthProducts] = useState<Product[]>([]);
  const navigate = useNavigate();

  useEffect(() => {
    // Filter products by health category
    const filtered = products.filter(p => p.category === 'health');
    setHealthProducts(filtered);
  }, []);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <main className="flex-grow container mx-auto px-4 py-6">
        <h1 className="text-2xl font-semibold text-gray-800 mb-4">Health</h1>
